...
foreach ($gpo in $allGPOs)
{
    ...
    if ($gpo.user.DSVersion -gt 0)
    {
        ...
        {
            Set-GPPermission -Guid $gpo.Id -PermissionLevel GpoRead -TargetName "Authenticated Users" -TargetType Group #-WhatIf
            Write-Host $gpo.DisplayName "has been modified to grant Authenticated Users read access"
            $ChangedGPOs += @($gpo.DisplayName)
        }
    }

}
Write-Host $ChangedGPOs.Count "wurden geändert."